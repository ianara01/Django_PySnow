...
def menu_add(request):
    return render(request, 'cafe/menu_add.html')
...
def add_menu_data(request):
    ...
    if menu_name_from_form.strip()=='': # 제대로 된 값이 맞는지 검증해서 처리
        ...
        return render(request, 'cafe/menu_add.html', context)
    ...
    return render(request, 'cafe/menu_add.html') # 이후 단계 페이지 설정
...
def create(request):
    if request.method =="POST":
        ...
        return HttpResponseRedirect(reverse('cafe:manager_home'))
    # model = Menu
    # success_url = reverse('cafe:manager_home')
...
def delete(requst, pk):
    menu = Menu.objects.get(pk=pk)
    menu.delete()
    return HttpResponseRedirect(reverse('cafe:menu_list')) # success_url
